...
def train(args, data_info, show_loss):
    ...
    model = Akupm(args, n_entity, n_relation)
...
def get_feed_dict(args, model, data, ripple_set, start, end, add_user=False):
    feed_dict = dict()
...
def evaluation(sess, args, model, data, ripple_set, add_user=False, PR=False):
    start = 0
    auc_list = []
    acc_list = []
    if PR:
    ...
    while start < data.shape[0]:
        if PR:
            auc, acc,pre,rec,f1 = model.eval(sess, get_feed_dict(args, model, data, ripple_set, start, start + args.batch_size),PR,args.k)
            pre_list.append(pre)
            rec_list.append(rec)
            f1_list.append(f1)
        else:
            auc, acc = model.eval(sess, get_feed_dict(args, model, data, ripple_set, start, start + args.batch_size))
        auc_list.append(auc)
        acc_list.append(acc)
        start += args.batch_size
    if PR:
